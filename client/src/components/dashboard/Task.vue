  ...
  <div>
    ...
    >
      ...
      >
        ...
        >
          <i class="fas fa-ellipsis-h" style="color: #9155FD"></i>
        </span>
        <p v-if="element.expireAt" style="color: black">
          <i class="fa fa-clock" style="color: #9155FD"/>
          {{element.expireAt | moment('MMMM D')}}
        </p>
        <span
          :class="`badge badge-${element.labelType}`"
          style="text-transform:capitalize; font-size: 0.635rem; color: black"
          v-if="element.labelType"
        >
          <i class="fas fa-circle" style="color: #9155FD;" />
          {{element.label}}
        </span>
      </b-card>
    </draggable>
